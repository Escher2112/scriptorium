...
try {
  ...
  await check('apply_edit preserves the caret when the edit is elsewhere (FEAT-1)', () => E(`(async function(){
    newTab('# H\\n\\nAlpha first paragraph.\\n\\nBravo second paragraph.\\n','caret.md'); setView('write');
    var view=editor.wwEditor.view, st=view.state, TS=st.selection.constructor, doc=st.doc, pos=null;
    doc.descendants(function(n,p){ if(pos===null&&n.isText){ var k=n.text.indexOf('second'); if(k>=0) pos=p+k; } });
    if(pos===null) return false;
    view.dispatch(view.state.tr.setSelection(TS.create(view.state.doc,pos,pos)));   // caret parked in the 2nd paragraph
    var before=view.state.selection.from;
    var r=await execTool('apply_edit',{old_string:'Alpha first',new_string:'ALPHA FIRST'});  // plain edit in the 1st paragraph
    var after=editor.wwEditor.view.state.selection.from;
    // caret must NOT have jumped to the top; it should stay in the same logical spot (len is unchanged here)
    return r.indexOf('OK')===0 && editor.getMarkdown().indexOf('ALPHA FIRST paragraph')>=0 && after>5 && Math.abs(after-before)<=2;
  })()`));
  await check('apply_edit with markdown formatting still renders (falls back safely)', () => E(`(async function(){newTab('# H\\n\\nplain sentence here.\\n','fb.md'); setView('write'); var r=await execTool('apply_edit',{old_string:'plain sentence here.',new_string:'**bold sentence** here.'}); return r.indexOf('OK')===0 && !!document.querySelector('#tui .toastui-editor-ww-container strong');})()`));
  ...
  await check('dump_state answers about the app own saved state (tabs/chats/trace/settings; persona text only on request)', () => E(`(async function(){ var r=await docCommand('dump_state',{}); var a=r.ok && Array.isArray(r.result.tabs.list) && Array.isArray(r.result.chats.list) && Array.isArray(r.result.trace) && !!r.result.settings && typeof r.result.assistant.identity_chars==='number' && !('identity' in r.result.assistant); var b=await docCommand('dump_state',{keys:['tabs']}); return a && b.ok && !!b.result.tabs && !b.result.chats; })()`));
  ...
  section('views');
  await check('browser print header/footer (title, date, file path, page numbers) cannot appear on the printed page', async () => { const a=await cdp.send('Page.printToPDF',{preferCSSPageSize:true,displayHeaderFooter:false}); const b=await cdp.send('Page.printToPDF',{preferCSSPageSize:true,displayHeaderFooter:true}); const same=a.data.length===b.data.length; return {ok: same, info: same?'PDF byte-identical with headers on/off ('+Math.round(a.data.length*3/4/1024)+' KB)':'headers ON adds '+(b.data.length-a.data.length)+' b64 chars'}; });
  await check('print → PDF page count equals the Page-view sheet count (WYSIWYG print)', async () => { const n=await E(`(function(){ var d='# Print'+String.fromCharCode(10,10); for(var i=1;i<=14;i++){ d+='## Section '+i+String.fromCharCode(10,10)+'Body text filler to occupy vertical space across pages, enough words to wrap a couple of lines each time. '.repeat(1+(i%3))+String.fromCharCode(10,10)+'- one'+String.fromCharCode(10)+'- two'+String.fromCharCode(10,10); } src.value=d; setView('render'); return document.querySelectorAll('#paper .page').length; })()`); const r=await cdp.send('Page.printToPDF',{preferCSSPageSize:true,printBackground:false}); const pdf=Buffer.from(r.data,'base64').toString('latin1'); const pages=(pdf.match(/\/Type\s*\/Page[^s]/g)||[]).length; const box=(pdf.match(/\/MediaBox\s*\[\s*0\s+0\s+(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)/)||[]); const letter=Math.round(+box[1])===612&&Math.round(+box[2])===792; return {ok: pages===n && n>=3 && letter, info: n+' sheets -> '+pages+' PDF pages, box '+box[1]+'x'+box[2]}; });
  await check('BUG-1: an image-block document keeps every image when printed (one image per sheet, no silent clip)', async () => { const md=readFileSync(join(ROOT,'test','fixtures','bug1-image-blocks.md'),'utf8'); const n=await E(`(async function(){ src.value=${JSON.stringify(md)}; setView('render'); for(var t=0;t<80;t++){ var im=[].slice.call(document.querySelectorAll('#paper img')); if(im.length && im.every(function(x){return x.complete && x.naturalHeight;})) break; await new Promise(function(r){setTimeout(r,50);}); } await new Promise(function(r){setTimeout(r,150);}); return document.querySelectorAll('#paper .page').length; })()`); const r=await cdp.send('Page.printToPDF',{preferCSSPageSize:true,printBackground:false}); const pdf=Buffer.from(r.data,'base64').toString('latin1'); const pages=(pdf.match(/\/Type\s*\/Page[^s]/g)||[]).length; const count=+((pdf.match(/\/Count\s+(\d+)/)||[])[1]||0); return {ok: n===6 && (pages===6 || count===6), info: n+' page divs, PDF '+pages+' pages, /Count '+count}; });
