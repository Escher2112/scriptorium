...
try {
  ...
  section('views');
  await check('browser print header/footer (file name, path, date) cannot appear on the printed page', async () => { const zlib=await import('node:zlib'); const r=await cdp.send('Page.printToPDF',{preferCSSPageSize:true,displayHeaderFooter:true}); const buf=Buffer.from(r.data,'base64'); let text=''; let i=0; while((i=buf.indexOf('stream',i))>=0){ const s=buf.indexOf('\n',i)+1; const e=buf.indexOf('endstream',s); if(e<0) break; try{ text+=zlib.inflateSync(buf.subarray(s,e)).toString('latin1'); }catch(err){ text+=buf.subarray(s,e).toString('latin1'); } i=e+9; } const leaked=/127\.0\.0\.1|scriptorium\.html/.test(text); return {ok: !leaked, info: leaked?'URL text found in PDF streams':'no URL/filename in PDF'}; });
